...
class GeneralLoRALoader:
    ...
    def split_trainable_modules(self, state_dict):
        lora_state_dict = {}
        other_state_dict = {}
    ...
    def load_lora_trainable_modules(self, model: torch.nn.Module, state_dict_lora, alpha=1.0):
        updated_num = 0
    ...
    def load_other_trainable_modules(self, model: torch.nn.Module, state_dict_lora_remain):
        updated_num = 0
        for name, param in model.named_parameters():
            if name in state_dict_lora_remain:
                if param.shape == state_dict_lora_remain[name].shape:
                    param.data.copy_(state_dict_lora_remain[name])
                    updated_num += 1
        print(f"{updated_num} tensors are updated by other LoRA.")
